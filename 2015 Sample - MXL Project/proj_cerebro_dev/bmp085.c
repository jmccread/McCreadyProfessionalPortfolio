...
void bmp085ReadUT(void)
{
  ...
  if (check_e()) e.bmp = I2C_Write(&ut_tx_data, 0x02, BMP085_ADDR);
...
void bmp085ReadUP()
{
  ...
  if (check_e()) e.bmp = I2C_Write(&up_tx_data, 0x02, BMP085_ADDR);
}
...
{
  uint8_t data[2]; 
  if (check_e())  e.bmp = I2C_Write(&address, 0x01, BMP085_ADDR);
  if (check_e()) e.bmp = I2C_Read(&data, 0x02, BMP085_ADDR);
...
long bmp085GetPressure(void)
{
  ...
  if (check_e()) e.bmp =I2C_Write(&up_tx_data[0], 0x01, BMP085_ADDR);
  if (check_e()) e.bmp = I2C_Read(&data, 2, BMP085_ADDR);
