 ...
 *                  - float * data -- pointer to float datums to be sent via uart
 ...
 *
 * Side Effects:    Writes data over USCI A1 via uart according to the configuration in 
 *                  start_device_uart
 *
 ...
 *                  
 *************************************************************************************/
uint8_t uart_transmit_float(usci_a_uart_t *in_ptr,  
                                   float * data, uint8_t length)
{
  // Variables
  ...
  for (i=0; i<length; i++) {
    ...
    memcpy(buff, &data[i], sizeof(float));
 ...
 *                  - float * data -- pointer to uint32_t datums to be sent via uart
                               ...
                               uint32_t * data, uint8_t length)
{
  ...
  for (i=0; i<length; i++) {
    ...
    memcpy(buff, &data[i], sizeof(uint32_t));
                                   ...
                                   int32_t * data, uint8_t length)
{
  ...
  // Load all the data 
  for (i=0; i<length; i++) {
    memcpy(buff, &data[i], sizeof(int32_t));
    // load buffer into ring buffer
    rbuffer_putchars(&UART_TX, buff, 4);
    }
  // End data 
 ...
 *                  - float * data -- pointer to int16 datums to be sent via uart
 ...
 *                  
 *************************************************************************************/
uint8_t uart_transmit_int16(usci_a_uart_t *in_ptr, int16_t * data, uint8_t length)
{
  // Variables
  ...
  for (i=0; i<length; i++) {
    ...
    memcpy(buff, &data[i], sizeof(int16_t));
 ...
 *                  
 *************************************************************************************/
uint8_t uart_transmit_uint16(usci_a_uart_t *in_ptr,uint16_t * data, uint8_t length)
{
  // Variables
  uint8_t buff[2]; 
  uint8_t checksum[2];  
  ...
  for (i=0; i<length; i++) {
    // Most Significant byte sent first;
    memcpy(buff, &data[i], sizeof(uint16_t));
    // load buffer into ring buffer
...
/*************************************************************************************
 * Function:        uint8_t uart_transmit_uint8(usci_a_uart_t *in_ptr,  
 ...
 *                  
 *************************************************************************************/
uint8_t uart_transmit_uint8(usci_a_uart_t *in_ptr, uint8_t * data, uint8_t length)
{
  // Variables
  uint8_t checksum[2];  
  uint8_t e, i;
  ...
  for (i=0; i<length; i++) {
    ...
    rbuffer_putchar(&UART_TX, data[i]);
    }
  // End data 
  rbuffer_putchar(&UART_TX, UART_STOP_BYTE);
 ...
 *                  
 *************************************************************************************/
uint8_t uart_transmit_int8(usci_a_uart_t *in_ptr, int8_t * data, uint8_t length)
{
  // Variables
  uint8_t checksum[2];  
  uint8_t e, i;
  ...
  for (i=0; i<length; i++) {
    // load data into ring buffer
    rbuffer_putchar(&UART_TX, (uint8_t)data[i]);
    }
  // End data 
  rbuffer_putchar(&UART_TX, UART_STOP_BYTE);
 ...
 *                  
 *************************************************************************************/
uint8_t uart_recieve_float(usci_a_uart_t *in_ptr, uint32_t mclk_speed, float * data ) {
    ...
    while(rbuffer_count(&UART_RX))
    {
     ...
     switch(state) 
      ...
      case 2 : 
        ...
        state =3; 
        break; 
      // Get all the individual bytes to assemble into a data piece 
      case 3 : 
         switch(i) {
          case 0: 
            temp[i] = buff;
          ...
          case 3: 
            temp[i] = buff;
            i = 0;
            memcpy(&data[j], temp, sizeof(float));
            j++; 
            if (j == (length)) state = 4; 
            break;
          }
      ...
      // Check for stop byte
      case 4: 
        if (buff == UART_STOP_BYTE) state = 5;  
        else return 0 ; 
        break; 
      // Get the checksum then check it. 
      case 5: 
        RXd_CS[0] = buff; state= 6; 
        break;
      case 6:
        RXd_CS[1] = buff; state=7; 
 ...
 *                                               uint32_t mclk_speed, uint32_t * data )
 *
 * PreCondition:      1. start_device_uart has run
 *                    2. UART data has been sent and has been recieved by msp430
 *
 ...
 *                    - usci_a_uart_t *in_ptr: Uart A bus being used
 *                    - uint32_t mclk_speed: essentially a timeout for the wait uart busy fn.
 *                    - uint32_t * data: Pointer to where the received data is going 
 *  
 * Output:            1 if UART has correctly been received according to 'protocol' in UART.h
 *                    0 if something broke... 
 *
 * Side Effects:      Writes to uint32_t * data 
 *  
 * Overview:          1. Waits for UART_Not_Busy to run with timeout
 *                    2. Cycles through ring buffer where recieved uart bytes are stored
 *                    3. Checks bytes and checksum, returns 0 if something didn't match
 ...
 *                  
 *************************************************************************************/
uint8_t uart_recieve_uint32(usci_a_uart_t *in_ptr, uint32_t mclk_speed, uint32_t * data ) {
 ...
 *                                               uint32_t mclk_speed, int32_t * data )
 *
 * PreCondition:      1. start_device_uart has run
 *                    2. UART data has been sent and has been recieved by msp430
 *
 ...
 *                    - usci_a_uart_t *in_ptr: Uart A bus being used
 *                    - uint32_t mclk_speed: essentially a timeout for the wait uart busy fn.
 *                    - int32_t * data: Pointer to where the received data is going 
 *  
 * Output:            1 if UART has correctly been received according to 'protocol' in UART.h
 *                    0 if something broke... 
 *
 * Side Effects:      Writes to int32_t * data 
 *  
 * Overview:          1. Waits for UART_Not_Busy to run with timeout
 *                    2. Cycles through ring buffer where recieved uart bytes are stored
 *                    3. Checks bytes and checksum, returns 0 if something didn't match
 ...
 *                  
 *************************************************************************************/
uint8_t uart_recieve_int32(usci_a_uart_t *in_ptr, uint32_t mclk_speed, int32_t * data ) {
 ...
 *                                               uint16_t mclk_speed, int16_t * data )
 *
 * PreCondition:      1. start_device_uart has run
 *                    2. UART data has been sent and has been recieved by msp430
 *
 ...
 *                    - usci_a_uart_t *in_ptr: Uart A bus being used
 *                    - uint32_t mclk_speed: essentially a timeout for the wait uart busy fn.
 *                    - int16_t * data: Pointer to where the received data is going 
 *  
 * Output:            1 if UART has correctly been received according to 'protocol' in UART.h
 *                    0 if something broke... 
 *
 * Side Effects:      Writes to uint16_t * data 
 *  
 * Overview:          1. Waits for UART_Not_Busy to run with timeout
 *                    2. Cycles through ring buffer where recieved uart bytes are stored
 *                    3. Checks bytes and checksum, returns 0 if something didn't match
 ...
 *                  
 *************************************************************************************/
uint8_t uart_recieve_uint16(usci_a_uart_t *in_ptr, uint32_t mclk_speed, uint16_t * data ) {
      ...
      case 3 : 
         switch(i) {
          ...
          case 1: 
            temp[i] = buff;
            i = 0;
            memcpy(&data[j], temp, sizeof(int16_t));
            j++; 
            if (j == (length)) state = 4; 
            break;
          }
...
/*************************************************************************************
 * Function:          uint8_t uart_recieve_int16(usci_a_uart_t *in_ptr, 
 *                                               uint32_t mclk_speed, int16_t * data )
 *
 * PreCondition:      1. start_device_uart has run
 *                    2. UART data has been sent and has been recieved by msp430
 *
 ...
 *                    0 if something broke... 
 *
 * Side Effects:      Writes to int16_t * data 
 *  
 * Overview:          1. Waits for UART_Not_Busy to run with timeout
 *                    2. Cycles through ring buffer where recieved uart bytes are stored
 *                    3. Checks bytes and checksum, returns 0 if something didn't match
 ...
 *                  
 *************************************************************************************/
uint8_t uart_recieve_int16(usci_a_uart_t *in_ptr, uint32_t mclk_speed, int16_t * data ) {
 ...
 *                                               uint32_t mclk_speed, int8_t * data )
 *
 * PreCondition:      1. start_device_uart has run
 *                    2. UART data has been sent and has been recieved by msp430
 *
 ...
 *                    - usci_a_uart_t *in_ptr: Uart A bus being used
 *                    - uint32_t mclk_speed: essentially a timeout for the wait uart busy fn.
 *                    - int8_t * data: Pointer to where the received data is going 
 *  
 * Output:            1 if UART has correctly been received according to 'protocol' in UART.h
 *                    0 if something broke... 
 *
 * Side Effects:      Writes to int8_t * data 
 *  
 * Overview:          1. Waits for UART_Not_Busy to run with timeout
 *                    2. Cycles through ring buffer where recieved uart bytes are stored
 *                    3. Checks bytes and checksum, returns 0 if something didn't match
 ...
 *                  
 *************************************************************************************/
uint8_t uart_recieve_int8(usci_a_uart_t *in_ptr, uint32_t mclk_speed, int8_t * data ) {
...
/*************************************************************************************
 * Function:          uint8_t uart_recieve_int8(usci_a_uart_t *in_ptr, 
 *                                               uint32_t mclk_speed, uint8_t * data )
 *
 * PreCondition:      1. start_device_uart has run
 *                    2. UART data has been sent and has been recieved by msp430
 *
 ...
 *                  
 *************************************************************************************/
uint8_t uart_recieve_uint8(usci_a_uart_t *in_ptr, uint32_t mclk_speed, uint8_t * data ) {
