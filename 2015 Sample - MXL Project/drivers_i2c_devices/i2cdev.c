...
* date:	07/23/2014
*/
// TO DO: update errors if nessicary 
#include "i2cdev.h"
...
uint8_t readBytes(uint8_t devAddr, uint8_t regAddr, uint8_t length, uint8_t *data, uint16_t timeout) 
  ...
  uint8_t err;
  uint16_t length16=0; 
  err = I2C_Write(&regAddr, 0x01, devAddr);
  length16 = (uint16_t) length;
  if (!err) {
    err = I2C_Read(data, length16, devAddr);
    }
  return err; 
  }
...
uint8_t readWords(uint8_t devAddr, uint8_t regAddr, uint8_t length, uint16_t *data, uint16_t timeout) {
  ...
  if (!err)
  {
    err = I2C_Read(data, (length16*2), devAddr);
  }
  else return err; 
  num_bytes_got = strlen(buffer);
  /*
  #ifdef LittleEndian
    for(count=0, count<length, count++) {
      *data[count]=(int16_t)buffer[index+1] << 8 | (int16_t)buffer[index]);
      index = index+2; 
      }
  #endif 
  */
...
int writeBytes(uint8_t devAddr, uint8_t regAddr, uint8_t length, uint8_t* data) {
  uint8_t status = 0;
  uint8_t i =0; 
  const uint16_t length16 = (uint16_t)length+1; 

  // Avoid dynamic memory allocation 
  uint8_t data_cat[I2C_MAX_BUFFER_LENGTH];

  // Concatinate the register address and the data to be written to it. 
  for( i = 0; i<length16; i++) {
    if (i==0) data_cat[i] = regAddr; 
    else data_cat[i] = data[i-1];
     }
  status = I2C_Write(data_cat, length16, devAddr); 
...
int writeWords(uint8_t devAddr, uint8_t regAddr, uint8_t length, uint16_t* data) {
		...
		#ifdef LittleEndian
			for(count=0, count<length, count++) {
				buffer[index]=  (uint8_t)(*(data+count)& 0x00FF); 
				buffer[index+1]=  (uint8_t)((*(data+count)>>8) & 0x00FF);
				index = index+2; 
				}
		#endif 
		...
		#ifdef BigEndian
			for(count=0, count<length, count++) {
				buffer[index+1]=  (uint8_t)(*(data+count)& 0x00FF); 
				buffer[index]=  (uint8_t)((*(data+count)>>8) & 0x00FF);
				index = index+2; 
				}
		#endif
		
	 status = writeBytes(devAddr, regAddr, length*2, &buffer);
